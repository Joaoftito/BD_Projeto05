...
SELECT filmeId, CONVERT(CHAR(10),dataFabricacao, 103) AS Fab FROM Dvd
WHERE dataFabricacao >= '01-01-2020'

SELECT dvdNum, dataLocacao, dataDevolucao, valor, CAST(valor + 2.00 AS DECIMAL(7, 2)) AS 
       valorMulta FROM Locacao
...
SELECT numCadastro, nome, logradouro + ' ' + CAST(num AS VARCHAR(50)) + ' ' + cep AS endComp 
FROM Cliente
WHERE numCadastro >= 5503